...
//
// Threading model:
//   Event thread — calls tick(), which iterates the active modulator table
//   under a lock-free RCU read guard.  tick() never blocks for control
//   operations.
//
...
public:
  ...
  // Tick all active modulators in insertion order and invoke cb for each
  // output. Lock-free on the event thread; cb is invoked inside the RCU
  // read-side CS. Outputs are recorded into last_outputs_ for cross-modulator
  // graph references.
  template <typename Fn> void tick(double beat, float tick_rate_hz, Fn &&cb) {
    auto guard = table_.read();
    if (!guard)
      return;
    for (const auto &id : guard->tick_order) {
      auto it = guard->mods.find(id);
      if (it == guard->mods.end())
        continue;
  ...
  // Number of active modulators (acquires write mutex; not for use on event
  // thread).
  std::size_t size() const;

  // Pre-warm the liburcu reader registration for the calling thread.
  // Must be called once from each thread that will call tick() before that
  // thread becomes realtime-scheduled or a CLAP process() callback.
  // See rcu.hpp: pre_warm_rcu_reader() for the full contract.
  void pre_warm_reader() noexcept { pre_warm_rcu_reader(); }

private:
  using mod_map =
      std::unordered_map<std::string, std::shared_ptr<abstract_modulator>>;
