...
namespace {
    ...
    constexpr uint32_t kSeeds[8] = {
        0xACE1u, 0xDEADu, 0xBEEFu, 0xCAFEu, 0x1234u, 0x5678u, 0x9ABCu, 0xDEF0u,
    };
...
void fractal_modulator::update(std::string_view key, float value) {
    if (key == "base_rate")
        base_rate_ = std::clamp(value, 0.001f, 100.0f);
    else if (key == "octaves")
        octaves_ = std::clamp(static_cast<int>(std::round(value)), 1, kMaxOctaves);
    else if (key == "lacunarity")
        lacunarity_ = std::clamp(value, 1.1f, 8.0f);
    else if (key == "persistence")
        persistence_ = std::clamp(value, 0.01f, 2.0f);
    else if (key == "shape")
        shape_ = std::clamp(static_cast<int>(std::round(value)), 0, 2);
    else if (key == "threshold")
        threshold_ = std::clamp(value, -1.0f, 1.0f);
    else if (key == "depth")
...
float fractal_modulator::sample(int i) const noexcept {
    switch (shape_) {
    case 1:
        return 1.0f - 4.0f * std::abs(phase_[i] - 0.5f); // triangle
    case 2:
        return held_[i]; // S&H
    default:
        return std::sin(kTwoPi * phase_[i]); // sine
    }
}
