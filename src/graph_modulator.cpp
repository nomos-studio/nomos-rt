...
namespace {
    ...
    int parse_node(const edn::value& v, control_graph& g) {
        if (v.is<double>()) {
            g.nodes.push_back(
                {.type = cg_node::kind::const_val, .args = {static_cast<float>(v.get<double>())}});
            return static_cast<int>(g.nodes.size()) - 1;
        }
        if (v.is<int64_t>()) {
            g.nodes.push_back(
                {.type = cg_node::kind::const_val, .args = {static_cast<float>(v.get<int64_t>())}});
            return static_cast<int>(g.nodes.size()) - 1;
        }
        if (!v.is<edn::vector>())
            return -1;
        ...
        if (items.empty() || !items[0].is<edn::keyword>())
            return -1;
        ...
        if (op == "const") {
            ...
            node.args[0] = (items.size() > 1 && items[1].is<double>())
                               ? static_cast<float>(items[1].get<double>())
                               : 0.0f;
    ...
    float eval_node(cg_node& n, const std::vector<float>& vals, const control_graph& g,
                    const modulator_engine* engine, double beat, float dt) {
        ...
        case cg_node::kind::sin:
            return std::sin(in(0) * kTwoPi);
        case cg_node::kind::cos:
            return std::cos(in(0) * kTwoPi);
        ...
        case cg_node::kind::add:
            return in(0) + in(1);
        case cg_node::kind::mul:
            return in(0) * in(1);
        case cg_node::kind::neg:
            return -in(0);
        case cg_node::kind::abs_val:
            return std::abs(in(0));
...
graph_modulator::graph_modulator(control_graph graph, const modulator_engine* engine)
    : graph_(std::move(graph)), engine_(engine), values_(graph_.nodes.size(), 0.0f) {
}
...
modulator_output graph_modulator::tick(double beat, float tick_rate_hz) {
    ...
    if (graph_.out_cv >= 0)
        out.cv = values_[graph_.out_cv];
    if (graph_.out_aux >= 0)
        out.aux = values_[graph_.out_aux];
    if (graph_.out_gate >= 0)
        out.gate = values_[graph_.out_gate] > 0.5f;
    if (graph_.out_gate2 >= 0)
        out.gate2 = values_[graph_.out_gate2] > 0.5f;
    return out;
}
