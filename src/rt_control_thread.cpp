...
std::vector<uint8_t>
mts_bulk_dump(const std::unordered_map<int, double> &tuning, uint8_t prog,
              uint8_t device_id) {
...
clap_event_union make_note_event(bool is_on, int16_t key, int16_t channel,
                                 int16_t port, int16_t note_id,
...
rt_control_thread::rt_control_thread(config cfg, param_queue &queue,
                                     input_event_queue &in_queue)
    : cfg_(std::move(cfg)), queue_(queue), in_queue_(in_queue) {}

rt_control_thread::~rt_control_thread() { stop(); }
...
void rt_control_thread::dispatch_message(int conn_fd, const ipc::message &msg,
  ...
  case ipc::msg_register_source: {
    ...
    sess->register_source({
        ...
        .description =
            dsc && dsc->is<std::string>() ? dsc->get<std::string>() : "",
    });
    break;
  }
  ...
  case ipc::msg_note_off: {
    ...
    auto ev = make_note_event(is_on, get_i16("key", 60), get_i16("channel", 0),
                              get_i16("port", 0), get_i16("note-id", -1),
                              get_dbl("velocity", 0.0));
  ...
  case ipc::msg_schedule_bundle: {
    ...
    for (const auto &item : events_v->get<edn::vector>().items) {
      ...
      auto ev = make_note_event(is_on, static_cast<int16_t>(get_i("key", 60)),
                                static_cast<int16_t>(get_i("channel", 0)),
                                static_cast<int16_t>(get_i("port", 0)),
                                static_cast<int16_t>(get_i("note-id", -1)),
                                get_d("velocity", 0.0));
  ...
  case ipc::msg_midi_in: {
    ...
    ev.midi.port_index = (port_v && port_v->is<int64_t>())
                             ? static_cast<uint16_t>(port_v->get<int64_t>())
                             : 0;
  ...
  case ipc::msg_modulator_start: {
    ...
    if (type_name == "slope") {
      ...
      static const std::unordered_map<std::string, segment_modulator::type>
          type_map{
      ...
      shift_register_modulator::mode sr_mode =
          shift_register_modulator::mode::turing;
      ...
      auto mod =
          std::make_unique<shift_register_modulator>(sr_mode, length, dac_bits);
      ...
      cfg_.mod_engine->start(
          std::move(id), std::make_unique<graph_modulator>(
      ...
      cv_channel_decoder::source_field field =
          cv_channel_decoder::source_field::cv;
      ...
      auto mod = std::make_unique<cipher_modulator>(
          cfg_.mod_engine, get_src("clock"), get_src("data1"), get_src("data2"),
          get_src("strobe"));
      cfg_.mod_engine->start(std::move(id), std::move(mod));
      ...
      static const std::unordered_map<std::string, bools_ring_modulator::mode>
          mode_map{
      ...
      auto mod = std::make_unique<bools_ring_modulator>(
          br_mode, cfg_.mod_engine, get_in_src("in0"), get_in_src("in1"),
      ...
      auto mod = std::make_unique<sloth_chaos_modulator>(v, cfg_.mod_engine,
                                                         get_sloth_src("cv"));
      ...
      auto mod = std::make_unique<squid_axon_modulator>(
          cfg_.mod_engine, get_squid_src("clock"), get_squid_src("in1"),
      ...
      auto apply_vec_param = [&](const char *kw, const char *prefix) {
        ...
        for (int i = 0; i < (int)items.size() && i < genie_modulator::kMaxN;
             ++i) {
          if (items[i].is<double>()) {
      ...
      auto mod = std::make_unique<lets_splosh_modulator>(
          cfg_.mod_engine, get_ins_src("c"), get_ins_src("t"), get_ins_src("n"),
          get_ins_src("b"));
      cfg_.mod_engine->start(std::move(id), std::move(mod));
  ...
  case ipc::msg_cc: {
    ...
    auto get_u8 = [&](const char *kw, uint8_t def) -> uint8_t {
      const auto *v = m.find_kw(kw);
      return (v && v->is<int64_t>()) ? static_cast<uint8_t>(v->get<int64_t>())
                                     : def;
    };
  ...
  case ipc::msg_pitch_bend: {
    ...
    const int64_t raw =
        std::max(INT64_C(0), std::min(INT64_C(16383), get_i("value", 8192)));
...
void rt_control_thread::dispatch_extension(int, const ipc::message &,
