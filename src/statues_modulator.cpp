...
statues_modulator::statues_modulator(const modulator_engine* engine, std::string in_source_id,
                                     std::string addr_source_id)
    : engine_(engine), in_source_id_(std::move(in_source_id)),
...
modulator_output statues_modulator::tick(double /*beat*/, float /*tick_rate_hz*/) {
    ...
    if (engine_ && !addr_source_id_.empty()) {
        ...
        if (out)
            addr = static_cast<int>(out->state) & 0x7;
    } else if (use_addr_bits_) {
        addr = (addr_bits_[0] ? 1 : 0) | (addr_bits_[1] ? 2 : 0) | (addr_bits_[2] ? 4 : 0);
    }
...
void statues_modulator::update(std::string_view key, float value) {
    if (key == "in")
        in_ = std::clamp(value, -1.0f, 1.0f);
    else if (key == "addr") {
        addr_          = std::clamp(static_cast<int>(std::round(value)), 0, 7);
        use_addr_bits_ = false;
    } else if (key == "addr0") {
        addr_bits_[0]  = value > 0.5f;
        use_addr_bits_ = true;
    } else if (key == "addr1") {
        addr_bits_[1]  = value > 0.5f;
        use_addr_bits_ = true;
    } else if (key == "addr2") {
        addr_bits_[2]  = value > 0.5f;
        use_addr_bits_ = true;
    }
}
