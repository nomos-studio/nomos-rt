...
divine_cmos_modulator::divine_cmos_modulator(const modulator_engine* engine, std::string clock1_src,
                                             std::string clock2_src)
    : engine_(engine), clock1_src_(std::move(clock1_src)), clock2_src_(std::move(clock2_src)) {
}

bool divine_cmos_modulator::read_gate(const std::string& src_id, bool& prev) const noexcept {
...
modulator_output divine_cmos_modulator::tick(double /*beat*/, float tick_rate_hz) {
    ...
    const float alpha =
        (tick_rate_hz > 0.0f && tc > 0.0f) ? std::exp(-1.0f / (tc * tick_rate_hz)) : 0.0f;
...
void divine_cmos_modulator::update(std::string_view key, float value) {
    if (key == "gain0")
        gains_[0] = std::clamp(value, 0.0f, 5.0f);
    else if (key == "gain1")
        gains_[1] = std::clamp(value, 0.0f, 5.0f);
    else if (key == "gain2")
        gains_[2] = std::clamp(value, 0.0f, 5.0f);
    else if (key == "gain3")
        gains_[3] = std::clamp(value, 0.0f, 5.0f);
    else if (key == "slew")
        slew_ = std::clamp(value, 0.0f, 1.0f);
    else if (key == "clock1_tick" && value > 0.5f)
        clock1_pending_ = true;
    else if (key == "clock2_tick" && value > 0.5f)
        clock2_pending_ = true;
}
