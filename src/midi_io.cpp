...
bool midi_io::open_port(unsigned int index) {
  try {
    ...
    if (index >= count) {
      std::fprintf(stderr,
                   "[midi] output port %u out of range (%u available)\n", index,
                   count);
      return false;
    }
    out_.openPort(index);
...
bool midi_io::open_virtual_port(const std::string &name) {
  try {
    ...
    std::fprintf(stderr, "[midi] open_virtual_port error: %s\n", e.what());
    return false;
  }
}
...
void midi_io::realtime(uint8_t byte) { send({byte}); }
...
bool midi_io::open_input_port(unsigned int index, input_event_queue &q) {
  try {
    ...
    in_.ignoreTypes(false, true,
                    true); // pass notes/CC; ignore sysex/timing/sensing
    in_.openPort(index);
...
bool midi_io::open_input_port_by_name(const std::string &name,
                                      input_event_queue &q) {
  const unsigned int count = in_.getPortCount();
  for (unsigned int i = 0; i < count; ++i) {
...
bool midi_io::input_is_open() const noexcept { return in_.isPortOpen(); }
