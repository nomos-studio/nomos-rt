...
cipher_modulator::cipher_modulator(const modulator_engine* engine, std::string clock_src,
                                   std::string data1_src, std::string data2_src,
                                   std::string strobe_src)
    : engine_(engine), clock_src_(std::move(clock_src)), data1_src_(std::move(data1_src)),
      data2_src_(std::move(data2_src)), strobe_src_(std::move(strobe_src)) {
}
...
bool cipher_modulator::read_gate_level(const std::string& src_id, float fallback) const noexcept {
    if (engine_ && !src_id.empty()) {
        const auto* out = engine_->last_output(src_id);
        if (out)
            return out->gate;
...
modulator_output cipher_modulator::tick(double /*beat*/, float /*tick_rate_hz*/) {
    ...
    if (clock_edge) {
        ...
        if (!data2_src_.empty() || data2_ >= 0.0f) {
            ...
            const bool d2 = !data2_src_.empty() ? read_gate_src(data2_src_) : (data2_ > 0.5f);
            bit_in        = d1 || d2;
        } else {
...
void cipher_modulator::update(std::string_view key, float value) {
    if (key == "data1")
        ...
        clock_pending_ = true;
    else if (key == "data2_enable")
        data2_ = (value > 0.5f) ? 0.0f : -1.0f;
}
