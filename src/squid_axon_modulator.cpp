...
squid_axon_modulator::squid_axon_modulator(const modulator_engine* engine, std::string clock_src,
                                           std::string in1_src, std::string in2_src,
                                           std::string in3_src)
    : engine_(engine), clock_src_(std::move(clock_src)), in1_src_(std::move(in1_src)),
      in2_src_(std::move(in2_src)), in3_src_(std::move(in3_src)) {
}

float squid_axon_modulator::read_cv_src(const std::string& src, float fallback) const noexcept {
...
modulator_output squid_axon_modulator::tick(double /*beat*/, float /*tick_rate_hz*/) {
    ...
    if (edge) {
        ...
        const float in3 =
            (!in3_patched_ && in3_src_.empty()) ? stages_[3] : read_cv_src(in3_src_, in3_);
...
void squid_axon_modulator::update(std::string_view key, float value) {
    if (key == "nl_fb")
        nl_fb_ = std::clamp(value, 0.0f, 4.0f);
    else if (key == "lin_fb")
        lin_fb_ = std::clamp(value, 0.0f, 1.0f);
    else if (key == "in1")
        in1_ = value;
    else if (key == "in2")
        in2_ = value;
        ...
        in3_ = value;
    else if (key == "in3_patched")
        in3_patched_ = (value > 0.5f);
