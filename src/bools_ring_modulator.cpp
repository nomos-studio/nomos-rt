...
bools_ring_modulator::bools_ring_modulator(mode m, const modulator_engine* engine, std::string src0,
                                           std::string src1, std::string src2, std::string src3,
                                           std::string sample_src)
    : mode_(m), engine_(engine),
      src_{std::move(src0), std::move(src1), std::move(src2), std::move(src3)},
      sample_src_(std::move(sample_src)) {
}

bool bools_ring_modulator::read_gate(const std::string& src_id, float fallback) const noexcept {
...
modulator_output bools_ring_modulator::tick(double /*beat*/, float tick_rate_hz) {
    ...
    // Sample / latch mode.
    if (sampled_) {
        const bool sgate = !sample_src_.empty() ? read_gate(sample_src_, 0.0f) : false;
...
void bools_ring_modulator::update(std::string_view key, float value) {
    if (key == "in0")
        ...
        sampled_ = (value > 0.5f);
    else if (key == "sample_tick" && value > 0.5f)
