...
void sloth_chaos_modulator::State::step(float h, float K, float z_bias, float cv_in) noexcept {
...
sloth_chaos_modulator::sloth_chaos_modulator(variant v, const modulator_engine* engine,
                                             std::string cv_src)
    : variant_(v), engine_(engine), cv_src_(std::move(cv_src)) {
}
...
modulator_output sloth_chaos_modulator::tick(double /*beat*/, float tick_rate_hz) {
    ...
    const int v_idx = (variant_ == variant::triple)   ? 0
                      ...
                      : (variant_ == variant::apathy) ? 1
                                                      : 2;

    if (variant_ != variant::triple) {
        const float eff_dt = (tick_rate_hz > 0.0f) ? kTimeDilation[v_idx] / tick_rate_hz : 0.0f;
        const float h      = eff_dt / 4.0f;
        for (int i = 0; i < 4; ++i)
            states_[v_idx].step(h, K, z_bias, cv_in);
    } else {
        for (int vi = 0; vi < 3; ++vi) {
            const float eff_dt = (tick_rate_hz > 0.0f) ? kTimeDilation[vi] / tick_rate_hz : 0.0f;
            const float h      = eff_dt / 4.0f;
            for (int i = 0; i < 4; ++i)
    ...
    if (variant_ != variant::triple) {
        ...
        out.state      = (states_[0].z > 0.0f ? 1u : 0u) | (states_[1].z > 0.0f ? 2u : 0u) |
...
void sloth_chaos_modulator::update(std::string_view key, float value) {
    if (key == "knob")
        knob_ = std::clamp(value, 0.0f, 1.0f);
    else if (key == "cv_in")
        cv_in_ = value;
}
