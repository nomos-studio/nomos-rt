...
TEST_CASE("graph_modulator: beat_phase with param-controlled period", "[graph_modulator]") {
    nomos::rt::graph_modulator m{parse("[:beat-phase [:param :period]]", {{"period", 2.0f}})};
...
TEST_CASE("graph_modulator: sample_hold captures on rising edge", "[graph_modulator]") {
    ...
    g.nodes.push_back({.type = nomos::rt::cg_node::kind::param_ref, .param_key = "gate"});
...
TEST_CASE("graph_modulator: threshold produces gate output", "[graph_modulator]") {
    ...
    nomos::rt::graph_modulator m{parse("{:cv [:phasor 1.0] :gate [:threshold [:phasor 1.0] 0.5]}")};
...
TEST_CASE("graph_modulator: mod-out reads preceding modulator in same tick", "[graph_modulator]") {
    ...
    eng.start("sink",
              std::make_unique<nomos::rt::graph_modulator>(parse("[:mod-out :src :cv]"), &eng));
...
TEST_CASE("graph_modulator: mod-out on unknown id returns zero", "[graph_modulator]") {
    ...
    float cv = 0.0f;
    eng.tick(0.0, 100.0f,
             [&](const std::string&, const nomos::rt::modulator_output& out) { cv = out.cv; });
