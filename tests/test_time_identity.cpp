...
TEST_CASE("time_identity: apply_if_ready is idempotent after promotion", "[time_identity]") {
    ...
    REQUIRE(!ti.apply_if_ready(4.0));
    REQUIRE(ti.current.bpm == Catch::Approx(140.0));
}

TEST_CASE("time_identity: before apply_at does not promote", "[time_identity]") {
    nomos::rt::time_identity ti;
    ti.current = {120.0, 0.0};
    ti.pending = nomos::rt::pending_transition{
        .tl       = {100.0, 8.0},
        .policy   = edn::keyword{"smooth"},
        .apply_at = 8.0,
    };

    REQUIRE(!ti.apply_if_ready(7.999));
    REQUIRE(ti.current.bpm == Catch::Approx(120.0));
    REQUIRE(ti.pending.has_value());
}
